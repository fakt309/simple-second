...
function DateToSsTimestamp(date) {
  return Math.round((1000/864)*(date-2*60*60*1000)+((1492*365+477*366)*100000000));
}

function Ss(str) {
  if (typeof str == "undefined") {
    this.timestamp = DateToSsTimestamp(Date.now())
    return this;
...
Ss.prototype.getDayOfYear = function() {
  return this.getMonth()*73+this.getDay();
}
Ss.prototype.getDayOfWeek = function() {
  if (this.getDayOfYear() == 365) {
    return 4;
  } else {
    return this.getDayOfYear()%5;
  }
}
Ss.prototype.getWeek = function() {
  if (this.getDayOfYear() == 365) {
    return parseInt(this.getDayOfYear()/5)-1;
  } else {
    return parseInt(this.getDayOfYear()/5);
  }
}
...
module.exports = {
  Ss,
  dateToSs: function(date) {
    return new Ss(DateToSsTimestamp(date))
  }
}
